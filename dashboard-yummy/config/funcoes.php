...
function listarRegistroUmParametro2($tabela, $campos, $id, $idparametro, $ativo)
{
    $conn = conectar();
    try {
        $conn->beginTransaction();
...
function listarTodosRegistros2($tabela, $campos, $idcampo, $id)
{
    $conn = conectar();
    try {
        $sqlLista = $conn->prepare("SELECT $campos FROM $tabela WHERE $idcampo = $id ");
        $sqlLista->execute();
        if ($sqlLista->rowCount() > 0) {
            return $sqlLista->fetchAll(PDO::FETCH_OBJ);
        } else {
...
function excluirRegistros($tabela, $nomeid, $id)
{
    ...
    try {
        ...
        return 'Não foi possível excluir os dados. Erro: ' . $e->getMessage();
    };
}

// alterar dados
function alterarRegistros($tabela, $campos, $nomeid, $id)
{
    $conn = conectar();
    try {
        $sqlLista = $conn->prepare("UPDATE $tabela SET $campos WHERE $nomeid = $id");
        $result = $sqlLista->execute();
        if ($result === false) {
            $conn->rollback();
            return false;
        ...
        return 'Não foi possível alterar os dados. Erro: ' . $e->getMessage();
    };
}
